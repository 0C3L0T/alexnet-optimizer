...
double LEVELS[] = { 500000,  667000,  1000000, 1200000, 1398000, 1512000, 1608000,
                    1704000, 1800000, 1908000, 2016000, 2100000, 2208000 };

double bigamp(double bf) {
  bf         = bf * 1e-3;
  double ans = (2.77904e-10 * bf * bf * bf - 8.72535e-7 * bf * bf + 0.00102561 * bf - 0.15);
  return ans;
}
...
double totalwatts(double lf, double bf, double butil, double gutil, double lutil) {
  return (BASELINE_AMPS + smlamp(lf) * lutil + bigamp(bf) * butil
          + BIG_GPU_SUPP_FACTOR * scalebf(bf) * std::max(0.0, gutil - butil) + GPU_AMP * gutil)
         * 5.0;
}
